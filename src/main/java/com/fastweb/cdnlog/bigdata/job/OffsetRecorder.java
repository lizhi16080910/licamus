package com.fastweb.cdnlog.bigdata.job;
...
public class OffsetRecorder {
    private static final Logger LOG = Logger.getLogger(OffsetRecorder.class);
    ...
    public OffsetRecorder(long time, long lastTime, List<String> topics, String brokerList) {
    ...
    public OffsetRecorder(long time, long lastTime, List<String> topics, String brokerList, String offsetRecorderPathLocal) {
        this.time = sdf.format(new Date(time * 1000));
        this.lastTime = sdf.format(new Date(lastTime * 1000));
        this.topics = topics;
        this.brokerList = brokerList;
    ...
    public OffsetRecorder(long time, long lastTime, List<String> topics, Map<String, String> kafkaParams) {
        this.time = sdf.format(new Date(time * 1000));
        this.lastTime = sdf.format(new Date(lastTime * 1000));
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 5) {
            System.out.println("OffsetRecorder <time> <lastTime>  <topics> <brokerList> <offsetRecorderPathLocal>");
            System.exit(1);
        }
        ...
        String offsetRecorderPathLocal = args[4];
        new OffsetRecorder(time, lastTime, topics, brokerList, offsetRecorderPathLocal).run();
    }
}
