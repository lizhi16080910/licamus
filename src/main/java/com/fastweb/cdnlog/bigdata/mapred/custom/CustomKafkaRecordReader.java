 ...
 */
public class CustomKafkaRecordReader extends RecordReader<CustomKafkaInputFomat.EtlKey, Text> {
    private static final Log LOG = LogFactory.getLog(CustomKafkaRecordReader.class);
    private Map<String, String> kafkaParams = null;
    private CustomKafkaInputFomat.EtlKey key = null;
    ...
    public CustomKafkaRecordReader() {
        this.kafkaParams = new HashMap<>();
    }

    @Override
    public void initialize(InputSplit split, TaskAttemptContext context) throws IOException, InterruptedException {
        kafkaParams.put(CustomKafkaInputFomat.METADATA_BROKER_LIST,context.getConfiguration().get(Constant.KAFKA_BROKERS));

        CustomKafkaSplit csplit = (CustomKafkaSplit)split;
    ...
    public boolean nextKeyValue() throws IOException, InterruptedException {
        if(iter.hasNext()){
            ...
            this.key = new CustomKafkaInputFomat.EtlKey(this.topic,this.partitionId,msg.offset());
    ...
    @Override
    public CustomKafkaInputFomat.EtlKey getCurrentKey() throws IOException, InterruptedException {
        return this.key;
    }
