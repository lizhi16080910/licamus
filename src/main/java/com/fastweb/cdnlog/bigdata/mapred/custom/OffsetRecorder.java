...
public class OffsetRecorder {
    ...
    public List<EtlRequest> getPreviousFromLocal() {
        ...
        return getPreviousFromLocal(path);
    }

    public static List<EtlRequest> getPreviousFromLocal(String path) {
       // String path = this.offsetRecorderPathLocal + File.separator + this.lastTime + File.separator + FILE_NAME;
        LOG.info("previous request file path is :" + path);
        List<CustomKafkaInputFomat.EtlRequest> list = new ArrayList<>();
    ...
    private List<CustomKafkaInputFomat.EtlRequest> readPrevious(String path, Configuration conf) throws IOException {
        List<CustomKafkaInputFomat.EtlRequest> list = new ArrayList<>();
    ...
    public static EtlRequest convert(String line) {
        ...
        return new CustomKafkaInputFomat.EtlRequest(topic, partition, startOffset, endOffset);
    }

    public static Map<TopicPartition, EtlRequest> convertToMap(List<EtlRequest> list) {
        Map<CustomKafkaInputFomat.TopicPartition, CustomKafkaInputFomat.EtlRequest> map = new HashMap<>();
        for (CustomKafkaInputFomat.EtlRequest top : list) {
            String topic = top.getTopic();
            int partition = top.getPartiton();
            map.put(new CustomKafkaInputFomat.TopicPartition(topic, partition), top);
        }
        return map;
    }
    ...
    public void run() throws Exception {
        String path = this.offsetRecorderPath + Path.SEPARATOR + this.time + Path.SEPARATOR + FILE_NAME;
        String pathLocal = this.offsetRecorderPathLocal + File.separator + this.time + File.separator + FILE_NAME;
...
//        Configuration conf = new Configuration();
//        writeCurrentRequest(conf, map, path);
    }

    public static void writeCurrentRequestLocal(Map<TopicPartition, EtlRequest> map, String outPath) {
